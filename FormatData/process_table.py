...
def get_table_finqa_timeFun(table_json, function):
    time_flag = 0
    ...
    for v in function.variables:
        if '*n-1' in v:
            program = program.replace(v, str(int(table_json['table_value'][v.replace('*n-1', '')][str(int(time) - 1)])))
    for v in function.variables:
        if '*n-1' not in v:
            num = table_json['table_value'][v.replace('*n', '')][time]
    ...
    gold_inds = {}
    for (index, v) in enumerate(all_v_list):
        if v in function.variable_without_time:
            gold_inds['table_' + str(index + 1)] = table_row_to_text(table[0], table[index + 1])
    ...
    for j in input_data:
        ...
        for fun in function_list:
            ...
            for v in fun.variable_without_time:
                ...
                break
            if flag == 0:
                finQA = get_table_finqa_timeFun(j, fun)
